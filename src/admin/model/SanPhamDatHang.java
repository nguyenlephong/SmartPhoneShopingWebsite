package admin.model;

public class SanPhamDatHang {
	private String soThuTu;
	private String maSanPham;
	private String tenSanPham;
	private String giaSanPham;
	private String congNgheManHinh;
	private String doPhanGiai;
	private String manHinhRong;
	private String matKinhCamUng;
	private String cameraTruoc;
	private String cameraSau;
	private String denFlash;
	private String heDieuHanh;
	private String tocDoCPU;
	private String ram;
	private String boNhoTrong;
	private String theNhoNgoai;
	private String mang;
	private String sim;
	private String wifi;
	private String gps;
	private String chatLieu;
	private String kichThuoc;
	private String trongLuong;
	private String dungLuongPin;
	private String loaiPin;
	private String urlHinhAnh;
	private String soLuong;
	public String getSoThuTu() {
		return soThuTu;
	}
	public void setSoThuTu(String soThuTu) {
		this.soThuTu = soThuTu;
	}
	public String getMaSanPham() {
		return maSanPham;
	}
	public void setMaSanPham(String maSanPham) {
		this.maSanPham = maSanPham;
	}
	public String getTenSanPham() {
		return tenSanPham;
	}
	public void setTenSanPham(String tenSanPham) {
		this.tenSanPham = tenSanPham;
	}
	public String getGiaSanPham() {
		return giaSanPham;
	}
	public void setGiaSanPham(String giaSanPham) {
		this.giaSanPham = giaSanPham;
	}
	public String getCongNgheManHinh() {
		return congNgheManHinh;
	}
	public void setCongNgheManHinh(String congNgheManHinh) {
		this.congNgheManHinh = congNgheManHinh;
	}
	public String getDoPhanGiai() {
		return doPhanGiai;
	}
	public void setDoPhanGiai(String doPhanGiai) {
		this.doPhanGiai = doPhanGiai;
	}
	public String getManHinhRong() {
		return manHinhRong;
	}
	public void setManHinhRong(String manHinhRong) {
		this.manHinhRong = manHinhRong;
	}
	public String getMatKinhCamUng() {
		return matKinhCamUng;
	}
	public void setMatKinhCamUng(String matKinhCamUng) {
		this.matKinhCamUng = matKinhCamUng;
	}
	public String getCameraTruoc() {
		return cameraTruoc;
	}
	public void setCameraTruoc(String cameraTruoc) {
		this.cameraTruoc = cameraTruoc;
	}
	public String getCameraSau() {
		return cameraSau;
	}
	public void setCameraSau(String cameraSau) {
		this.cameraSau = cameraSau;
	}
	public String getDenFlash() {
		return denFlash;
	}
	public void setDenFlash(String denFlash) {
		this.denFlash = denFlash;
	}
	public String getHeDieuHanh() {
		return heDieuHanh;
	}
	public void setHeDieuHanh(String heDieuHanh) {
		this.heDieuHanh = heDieuHanh;
	}
	public String getTocDoCPU() {
		return tocDoCPU;
	}
	public void setTocDoCPU(String tocDoCPU) {
		this.tocDoCPU = tocDoCPU;
	}
	public String getRam() {
		return ram;
	}
	public void setRam(String ram) {
		this.ram = ram;
	}
	public String getBoNhoTrong() {
		return boNhoTrong;
	}
	public void setBoNhoTrong(String boNhoTrong) {
		this.boNhoTrong = boNhoTrong;
	}
	public String getTheNhoNgoai() {
		return theNhoNgoai;
	}
	public void setTheNhoNgoai(String theNhoNgoai) {
		this.theNhoNgoai = theNhoNgoai;
	}
	public String getMang() {
		return mang;
	}
	public void setMang(String mang) {
		this.mang = mang;
	}
	public String getSim() {
		return sim;
	}
	public void setSim(String sim) {
		this.sim = sim;
	}
	public String getWifi() {
		return wifi;
	}
	public void setWifi(String wifi) {
		this.wifi = wifi;
	}
	public String getGps() {
		return gps;
	}
	public void setGps(String gps) {
		this.gps = gps;
	}
	public String getChatLieu() {
		return chatLieu;
	}
	public void setChatLieu(String chatLieu) {
		this.chatLieu = chatLieu;
	}
	public String getKichThuoc() {
		return kichThuoc;
	}
	public void setKichThuoc(String kichThuoc) {
		this.kichThuoc = kichThuoc;
	}
	public String getTrongLuong() {
		return trongLuong;
	}
	public void setTrongLuong(String trongLuong) {
		this.trongLuong = trongLuong;
	}
	public String getDungLuongPin() {
		return dungLuongPin;
	}
	public void setDungLuongPin(String dungLuongPin) {
		this.dungLuongPin = dungLuongPin;
	}
	public String getLoaiPin() {
		return loaiPin;
	}
	public void setLoaiPin(String loaiPin) {
		this.loaiPin = loaiPin;
	}
	public String getUrlHinhAnh() {
		return urlHinhAnh;
	}
	public void setUrlHinhAnh(String urlHinhAnh) {
		this.urlHinhAnh = urlHinhAnh;
	}
	public String getSoLuong() {
		return soLuong;
	}
	public void setSoLuong(String soLuong) {
		this.soLuong = soLuong;
	}
	public SanPhamDatHang(String soThuTu, String maSanPham, String tenSanPham, String giaSanPham,
			String congNgheManHinh, String doPhanGiai, String manHinhRong, String matKinhCamUng, String cameraTruoc,
			String cameraSau, String denFlash, String heDieuHanh, String tocDoCPU, String ram, String boNhoTrong,
			String theNhoNgoai, String mang, String sim, String wifi, String gps, String chatLieu, String kichThuoc,
			String trongLuong, String dungLuongPin, String loaiPin, String urlHinhAnh, String soLuong) {
		super();
		this.soThuTu = soThuTu;
		this.maSanPham = maSanPham;
		this.tenSanPham = tenSanPham;
		this.giaSanPham = giaSanPham;
		this.congNgheManHinh = congNgheManHinh;
		this.doPhanGiai = doPhanGiai;
		this.manHinhRong = manHinhRong;
		this.matKinhCamUng = matKinhCamUng;
		this.cameraTruoc = cameraTruoc;
		this.cameraSau = cameraSau;
		this.denFlash = denFlash;
		this.heDieuHanh = heDieuHanh;
		this.tocDoCPU = tocDoCPU;
		this.ram = ram;
		this.boNhoTrong = boNhoTrong;
		this.theNhoNgoai = theNhoNgoai;
		this.mang = mang;
		this.sim = sim;
		this.wifi = wifi;
		this.gps = gps;
		this.chatLieu = chatLieu;
		this.kichThuoc = kichThuoc;
		this.trongLuong = trongLuong;
		this.dungLuongPin = dungLuongPin;
		this.loaiPin = loaiPin;
		this.urlHinhAnh = urlHinhAnh;
		this.soLuong = soLuong;
	}

	
 }
